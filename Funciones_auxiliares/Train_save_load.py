...
def load_and_plot(layers, train_data, test_data, nIter, model_file, loss_file, device='cpu', plots = True):
    # Recreate the model architecture
    model = NeuralNetwork(layers).to(device)

    # Load the trained weights
    loaded_model = load_model(NeuralNetwork, layers, model_file, device)

    # Load loss error
    loss_history = load_error(loss_file)
...
def train_and_save_model_S(model, data, loss_f, device, iter, lr, batch_size = None, plots = True):
    # Train the model
    trained_model, loss_history = prepare_training_S(model, data, loss_f, device, iter, lr, batch_size)
...
def prepare_training_S(model, data, loss_f, device, iter, lr, batch_size=None):
    """
    Prepares data and model for training
    """
    # Move model to device
    ...
    trained_model, loss_history = train_multi_MS_S(
            model, 
            A,
            loss_f, 
            S, 
...
def load_and_plot_S(layers, train_data, test_data, nIter, filename='trained_metabolic_model.pth', device='cpu', plots = True):
    # Recreate the model architecture
    model = NeuralNetwork(layers).to(device)
...
def calculate_MSE_S(model, train_data, test_data, device):
    # Calculate train loss and test loss
    train_loss_MS = torch.mean(torch.stack([loss_MS_Sv(model, i, train_data['S'], train_data['t_span'], train_data['lb'], train_data['ub'], train_data['index'], device) for i in train_data['A']]))
    test_loss_MS  = torch.mean(torch.stack([loss_MS_Sv(model, i, train_data['S'], train_data['t_span'], train_data['lb'], train_data['ub'], train_data['index'], device) for i in test_data]))
    train_loss = torch.mean(torch.stack([loss_rk4_S(model, i, train_data['S'], train_data['t_span'], train_data['lb'], train_data['ub'], train_data['index'], device) for i in train_data['A']]))
    test_loss  = torch.mean(torch.stack([loss_rk4_S(model, i, train_data['S'], train_data['t_span'], train_data['lb'], train_data['ub'], train_data['index'], device) for i in test_data]))
...
def calculate_MSE_simple(model, train_data, test_data, device):

    # Calculate train loss and test loss
    train_loss_MS = torch.mean(torch.stack([loss_simple(model, i, train_data['C'], train_data['t_span'], train_data['lb'], train_data['ub'], device) for i in train_data['A']]))
    test_loss_MS  = torch.mean(torch.stack([loss_simple(model, i, train_data['C'], train_data['t_span'], train_data['lb'], train_data['ub'], device) for i in test_data]))
    train_loss = torch.mean(torch.stack([loss_rk4_V2(model, i, train_data['C'], train_data['t_span'], train_data['lb'], train_data['ub'], device) for i in train_data['A']]))
    test_loss  = torch.mean(torch.stack([loss_rk4_V2(model, i, train_data['C'], train_data['t_span'], train_data['lb'], train_data['ub'], device) for i in test_data]))
